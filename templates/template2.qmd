---
title: "Template for Base Case with Futility"
format: html
editor: visual
---
...
-   A group sequential design is adopted
    ...
    -   A futility analysis is performed on `PFS` with a boundary 0.5 of `logrank` statistics.
...
action_at_final <- function(trial, event_name){
  ...
  stats <- rbind(pfs_result, os_result) %>% 
    mutate(order = c(1, 2, 2)) %>% 
    mutate(hypotheses = c('PFS', 'PFS', 'OS')) %>% 
    mutate(is_final = c(FALSE, TRUE, TRUE)) %>% 
    mutate(max_info = c(530, 530, 400))
  ...
  event_time <- c(trial$get_event_time('interim analysis'), 
                  trial$get_event_time('final analysis'))
  ...
  trial$save(value = event_time[test_result$order[test_result == 'PFS']], 
             name = 'pfs_decision_time')
  
  trial$save(value = event_time[test_result$order[test_result == 'OS']], 
...
We plan two trial events for interim and final analysis.
